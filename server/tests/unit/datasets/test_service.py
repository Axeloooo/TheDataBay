...
class FakeCSVLoader:
    ...
    def load(self) -> list[Document]:
        ...
        return [
            Document(page_content="age: 63, cp: 3", metadata={"source": self.file_path}),
            Document(page_content="age: 37, cp: 2", metadata={"source": self.file_path}),
        ]
...
async def test_embed_returns_completed_dataset_response(settings):
    ...
    assert harness.vectorstore.added_ids == [
        f"{response.listing_id}:0",
        f"{response.listing_id}:1",
    ]
