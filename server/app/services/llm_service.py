...
class LLMService:
    """Service for LLM operations like embedding generation."""
    ...
    def warmup_model(self) -> bool:
        ...
        try:
            ...
            ollama.embed(model=self.settings.embedding_model, input=test_text)
            return True
    ...
    def record_to_text(
        ...
        if len(column_names) < len(record):
            column_names.extend(
                [f"col_{i}" for i in range(len(column_names), len(record))]
            )
    ...
    async def generate_embeddings_chunked(
        self, texts: List[str], chunk_size: int = None
        ...
        chunk_size = chunk_size or self.settings.embedding_chunk_size
    ...
    def generate_single_embedding(self, text: str) -> Tuple[List[float], int]:
