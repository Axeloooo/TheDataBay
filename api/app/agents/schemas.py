...
class AgentCreateRequest(BaseModel):
    ...
    bio: Optional[str] = Field(
        None, description="Biography or description of the agent"
    )
    avatar_url: Optional[str] = Field(
        None, description="URL to the agent's avatar image"
    )
    ...
    owner_address: Optional[str] = Field(
        None, description="Ethereum address of the agent owner"
    )
...
class RecommendationResponse(BaseModel):
    ...
    similarity_score: Optional[float] = Field(
        ...
        ge=-1.0,
        le=1.0,
        description="Similarity score between agent and listing (-1 to 1)",
    )
    rationale: str = Field(..., description="Explanation for the recommendation")
    pros: List[str] = Field(
        default_factory=list, description="Pros or advantages of this listing"
    )
    cons: List[str] = Field(
        default_factory=list, description="Cons or disadvantages of this listing"
    )
...
class PurchaseRequestCreate(BaseModel):
    ...
    reason: str = Field(
        ..., max_length=500, description="Reason for the purchase request"
    )
...
class PurchaseRequestResponse(BaseModel):
    ...
    reviewed_at: Optional[datetime] = Field(
        None, description="Timestamp when the request was reviewed"
    )
    reviewed_by: Optional[str] = Field(
        None, description="Ethereum address of the reviewer"
    )
    created_at: datetime = Field(
...
class PurchaseRequestListResponse(BaseModel):
    ...
    requests: List[PurchaseRequestResponse] = Field(
...
class RecommendQueryRequest(BaseModel):
    ...
    query: str = Field(
        ..., description="Search query to find the best matching dataset"
    )
