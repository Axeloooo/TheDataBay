# TheDataBay FastAPI Backend

FastAPI backend service for TheDataBay AI workloads and API orchestration.
...
- `POST /api/v1/datasets/embed` - Upload a CSV, generate grounded dataset summaries, index them in PGVector, encrypt the raw dataset, upload it to IPFS, and return completed dataset metadata synchronously
...
- Loads a bounded CSV sample with LangChain `CSVLoader`
- Generates grounded summary profiles through the LLM domain
- Stores summary documents in the `dataset_summaries` PGVector collection
- Searches summary documents and returns whole marketplace datasets
...
- `LLM_PROVIDER` - LLM provider implementation (default: ollama)
- `LLM_BASE_URL` - Ollama-compatible base URL (default: http://localhost:11434)
- `LLM_CHAT_MODEL` - Chat model (default: deepseek-v4-flash:cloud)
- `LLM_EMBEDDING_MODEL` - Model for embeddings (default: nomic-embed-text)
- `LLM_EMBEDDING_DIMENSION` - Embedding vector dimension (default: 768)
- `LLM_THINK` - Enable model reasoning traces when supported (default: false)
- `OLLAMA_API_KEY` - Optional secret for hosted Ollama-compatible endpoints
- `DATASET_SUMMARY_COUNT` - Number of dataset summaries to generate (default: 5)
- `DATASET_SUMMARY_SAMPLE_ROWS` - Number of sample rows per summary (default: 20)
