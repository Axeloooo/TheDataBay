...
(pgvector, LLM provider, contract) is handled inside AIService — the router
...
def make_ranked_dataset(listing_id: str, score: float = 0.85) -> RankedDataset:
    return RankedDataset(
        dataset_id=f"dataset-{listing_id[-4:]}",
...
def test_similarity_search_returns_503_when_embedding_fails(client):
    raiser = RaisingAIService(EmbeddingError("LLM connection refused"))
...
def test_embed_query_returns_vector_payload(client, monkeypatch, settings):
    ...
    client.app.dependency_overrides[get_llm_service] = lambda: FakeLLMService()
...
def test_embed_query_validates_non_empty_query(client, monkeypatch):
    class FailLLMService:
        async def embed_text(self, query):
            raise AssertionError("embed_text should not be called")

    client.app.dependency_overrides[get_llm_service] = lambda: FailLLMService()
